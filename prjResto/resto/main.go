...
func main() {
	...
	// 1.
	var server pb.JadwalShiftServiceServer
	{
		...
		// to know this code, references from server
		...
		service := svc.NewJadwalShift(dbReadWriter)
		endpoint := ep.NewJadwalShiftEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCJadwalShiftServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterJadwalShiftServiceServer(grpcServer, server)
